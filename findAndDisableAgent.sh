...
agentUrl=`curl -k 'https://ec2-35-163-220-196.us-west-2.compute.amazonaws.com:8154/go/api/agents' -H 'Accept: application/vnd.go.cd.v4+json' | jq '._embedded.agents[] | select(.resources[] =='\"$resource_input\"') | (._links.self.href)'| tr -d '"'`
echo $agentUrl
